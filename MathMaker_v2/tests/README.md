...
against regression); and Through the Looking Glass (Wave 20 — the post-game
mirror replay, formerly Golden Numeria: the kingdom resets, the hero's
...
- `drive-castle.js` — Wave 7 (the ending): the castle gate (shut for a new
  ...
  replay-from-throne; and stepping through the looking glass (Wave 20 — the
  kingdom resets, the hero keeps everything, and the report card never
  re-locks).
...
- drive-understudy.js — Wave 13 echo plates (19 checks): the rolling 12-step buffer, first-summon intro modal (once-EVER), exact replay of the buffer, the theatrical half-beat, the wall-stop catching an overshoot ON the plate, the plate-hold powering the floor (annex + Cavern of Echoes), the gate staying open while held, the puzzle-locked pocket chest, the polite early stop (no modal, no cost), and the never-blocks place-swap. Screenshots include a 3-tile crown close-up.
- drive-myroom.js — Wave 13 Your Own Room (22 checks): pre-title blank-plate tease (masons note GONE), the named doorway drawing open + the once-ever opened-doorway prose, workbench counts, the mason's-trail lay/pickup with the protected entry tile, the 10-wall budget cap, build-solvable → pupil visibly solves → once-ever celebration + Miscount cameo (no gold — the room is the trophy), REBUILD-to-unsolvable → polite-stuck 💭 modal (costs nothing) → revise → solve again (no repeat modal), and the wedge nudge + reset cord inside the kid's own room.
- drive-staircase.js — Wave 13 homesick staircase (15 checks): lost at its authored Cavern spot (feet close-up screenshot), bump→following with the 🏠 pop + the monster-indifference log echo, exact two-move lag, battle-wait, overworld escort (screenshot), dungeon-entry parking, save/load mid-escort, the once-ever homecoming at the tower, the permanent "⤴ Start from floor 10" menu option landing on d22f9, and home = drawn nowhere.
- drive-court.js — Wave 14 "The Court" (23 checks): the herald 'Z' gated on endingDone (pre-ending "not yet"), the post-ending docket of 3 day-keyed cases led by the recurring Magistrate, a wrong ruling → baffled re-ask (never a scold, no HP cost, records the miss under the real skill, re-asks the SAME case), a correct ruling → settles + gratitude gift + cases-heard counter, the full 3/3 session celebration, the Court Clerk spawning at the first-session milestone (drawn as a live castle overlay, bump-to-hear), the queue stable within a day and re-rolling across days, and the Magistrate returning with a climbing visit counter. Screenshots: herald, petitioner mid-case, 3/3 celebration, spawned Faculty NPC.
- drive-parlor.js — Wave 15 "The Parlor" / the card game "Tiny Hats" (31 checks): the parlor door 'Z' gated on endingDone (pre-ending "not yet"), the once-ever dealer intro, the dealer hub with token/collection counters, a scripted match with a CAPTURE that flips an enemy card (red border → blue) and updates the running tally, the DETERMINISTIC opponent, winning banks a token + (forced) the opponent's card into the album, a LOSS costs zero tokens (never a scold), the two-digit parent dial widening every edge to 10..99, the "reach 20" dice side-table totalling correctly (bust pays zero, exactly-20 jackpots), the House Dealer joining the Faculty at its 3-game milestone (via the Wave 14 extension point, drawn as a live castle overlay), leaving the parlor landing beside its door, and the token shop refusing what you cannot afford (tokens never go negative). Screenshots: the board mid-match (foil hats + edge numbers legible at scale, never loose digits), a capture, the dealer NPC, the dice table, the House Dealer in the castle. (Pure card engine js/parlor.js — edge determinism, the capture rule, the opponent's full deterministic game, and the dice totals — is unit-tested by poisoning Math.random in tests/test.js.)
- drive-kitchen.js — Wave 16 "The Kitchen Garden" (34 checks): the garden door 'K' gated on endingDone (pre-ending "not yet"), the seed bench offering to plant, planting a 3×4 ARRAY that fills 12 seedlings, the count problem ("how many did you plant?") recording under muldiv_facts, a WRONG count met with a gentle re-ask (never a scold, no HP cost, records the miss, Next button), a correct count growing the patch, harvesting the array into 12 ingredients + the harvests counter, the sous-chef's greeting, a recipe drawing a real fraction problem (records under fractions_as/_m), a WRONG measure making a named DISASTER DISH (no HP loss, Next button) with the pet reacting (an emote pops), a correct measure making the real dish + a food grant (the food economy) + the dishes counter, the Gardener and Cook joining the Faculty at their milestones (via the Wave 14 extension point, live castle overlays, bump-to-hear), and leaving the garden landing beside its door. Screenshots: the planted array (a grid of plants, never loose digits), the count problem (in words), a recipe modal, a disaster dish (with the pet's emote), the sous-chef, a Faculty post. (The array/recipe generators, gentle-failure re-ask, food grant, faculty append, migration, and NG+ both directions are also unit-tested in tests/test.js.)
- drive-menagerie.js — Wave 17 "The Menagerie" (29 checks): the Menagerie door 'M' gated on endingDone (pre-ending "not yet"), the befriended roster POPULATING the pens (3 befriended kinds → 3 residents on distinct pen slots, each drawn from the catalog), bumping a resident offering to TEND it (and blocking its tile like an NPC), a tend drawing a weakest-first problem across the WHOLE capped skill set and RECORDING under its real skill, a WRONG tend met with a warm re-ask (never a scold, no HP cost, records the miss, does not grow the creature, Next button), a correct tend recording + advancing growth + bumping the up-only counters (tends / kinds tended), a creature STAGING UP (Settled) at the PET_STAGES-style threshold and earning a persistent tiny hat, the Menagerie Keeper joining the Faculty at its 2-kinds-tended milestone (via the Wave 14 extension point, a live castle overlay, bump-to-hear), the once-ever Parade firing when every present kind is tended (and NOT firing again — paradeSeen guard), leaving the nursery landing beside its door, and the ZERO-befriended empty-pen "room for a friend" state (not a crash). The drive switches the choice/clock skills off via parent topics so every tend is a typed-answer problem (which also exercises the parent-switch respect). Screenshots: the nursery with creatures, a tending problem, a creature in a tiny hat, the parade, the keeper, the empty-pen state. (Growth thresholds, hat determinism/persistence, weakest-first recording, the paradeSeen guard, the Keeper faculty append, migration, and NG+ both directions are also unit-tested in tests/test.js.)
- drive-avatar.js — Wave 18 "Choose Your Hero" (19 checks): the profile screen's own avatar picker sets the new game's form; the hero renders as each of the five pickable avatars (woman/man/dragon/fox/slug) PLUS the knight in the overworld (screenshot per form — every one reads as a HERO, distinct from enemies and the pet); mirror-for-facing on a non-human; a human avatar takes a skin/hair/outfit palette (palette-varied woman AND man screenshots); an earned tiny hat sits on a NON-HUMAN form (a dragon in a mortarboard); the Bag's Looking Glass is available from day one and changes the form LIVE with NO progress lost (gold/level/position unchanged); the Looking Glass shows its deadpan reflection + the world's deadpan reaction to a novel hero; the pet double-takes (🤔) on a change into a creature; the Study wardrobe (post-ending, bumped in the castle) opens the SAME picker wrapped in its personality (one state, two doors); the Adventurer's Passport round-trips s.avatar + s.avatarPalette + the worn hat; and a save with NO avatar field migrates to and renders the EXACT current knight frames (backward-compat is sacred). Screenshots: each avatar in the overworld, fox mirrored, palette-varied woman + man, dragon in a mortarboard, the profile picker, the Looking Glass, the Study wardrobe. (The roster/frames/palette map, DEFAULT-to-knight, newGame + setAvatar writing one state, Passport round-trip, and NG+ carry through startGolden AND returnToFinishedKingdom are also unit-tested in tests/test.js; every new avatar sprite passes the row/width/color validator.)
- drive-mirror.js — Wave 20 "Through the Looking Glass" (Looking Glass P1, the reversible mirror portal, 25 checks; extends/replaces the old drive-golden.js): the post-game replay is now a MIRROR world you step INTO and safely BACK OUT of — reusing the Golden Numeria machinery, reframed. Drives the SACRED reversibility: a finished kingdom reads as finished (castleOpen) and is NOT through the glass; the EXPLICIT entry prompt spells out you step into a reflection, promises the finished kingdom is kept safe / nothing lost / step-back-any-time, with the SAFE "Stay in my finished kingdom" as the primary/default; DECLINING leaves everything completely untouched; stepping through re-tangles the kingdom (tasks reset, isles re-locked, castle no longer finished) while the hero keeps level/badges/gear, snapshots the finished kingdom FIRST, sets the mirror flag/tint, and shows the persistent "through the looking glass" sidebar INDICATOR carrying a mirror-side step-back button; stepping back FROM INSIDE the mirror restores the finished kingdom EXACTLY (every task, isle, castleOpen again, snapshot consumed, indicator hidden); and the PARENT-PANEL recovery (mirror framing) restores identically. Screenshots: the entry prompt, the cool-tinted in-mirror overworld (reads as a reflection, still legible), the indicator close-up, the parent-panel mirror recovery. (The LOSSLESS round-trip field-by-field, the inMirror flag set-in-mirror/cleared-on-return, the safe default, and MIGRATION of a pre-Wave-20 Golden Numeria save into the mirror framing are also unit-tested in tests/test.js.)
- drive-mirror2.js — Wave 21 "aesthetic + the Cheshire Cat + Recognize" (Looking Glass P2, the content pass atop P1's cool tint, ~16 checks): everything gated on `E.inMirror()`, verified both ways (never leaks into normal play, always reachable through the glass, and stops again the moment the kid steps back). Drives the reversal-comedy pool (Farmer Fenwick's reversed "Goodbye!" greeting, chance pinned to 1 for determinism); the Cheshire Cat materializing at BOTH real thresholds (stepping through the glass for the first time; entering a mirror dungeon for the first time this reflection) with its line-counter advancing and NOT re-triggering on a repeat dungeon entry; the disappearing-smile FADE as a pure function of elapsed time (mid-fade: body faint/fading, smile still full alpha; smile-lingering: body fully gone, smile alone at full alpha; then the effect EXPIRES) — the fade's start timestamp is set only on dialog CLOSE, never while the modal hides it; a real mirror battle where the stance banner reads "Reflected" (not "Frightened"), the round prompt and calm-bar label read "Recognize" wording, and the victory panel reads "RECOGNIZED" (not "CALMED") — while the MECHANIC is proven byte-identical to Soothe (the monster still becalms at full health, pacified, and still joins the befriended axis); the ⚙️ Settings "Your way" section reflecting "recognizing" through the glass; and, stepping back, every one of the above reverting to its normal-play wording (Fenwick's plain greeting, "Frightened" in a fresh battle). Screenshots: the Cheshire's cat line, mid-fade, smile-lingering, the reversed-NPC modal, the Recognize battle banner/labels. (The pure timing function `E.cheshireAlphas` — including Calm Mode's gentler/instant cut — plus the reversal-pool/Cheshire/Recognize gating and `MM.data.recognizeLine`'s reunion-vs-new wording are also unit-tested in tests/test.js.)
- drive-negparse.js — Wave 19 "negatives in the answer parser" (Looking Glass P0, 10 checks): injects a SYNTHETIC negative-answer problem (`frac(-3,1)`) through MM.ui.showProblem and drives the real input→parse→check path — a kid TYPES "-3" (ASCII hyphen U+002D) and "−3" (the displayed unicode minus U+2212), both graded CORRECT and the verbatim answer passed through; the input box keeps a typed sign (no on-input sanitizer strips it); a POSITIVE "3" typed for a −3 answer is gently WRONG (the "Not quite" + solution re-ask, modal stays open, no crash, no JS errors). Pure parser capability — nothing generates negatives yet, so normal play is unchanged; the parser/reject cases (accepts "-3"/"−3"/"- 3"/"-1/2"/"-0.5"/"-2 1/2"/"+3", rejects "--3"/"3-"/bare "-", frac/feq already handle a negative numerator so checkAnswer needed no change, plus a dozen positive parses unchanged) are unit-tested in tests/test.js.
- drive-negatives.js — Wave 22 "negatives enter the mirror" (Looking Glass P3, 24 checks): the CORRECTNESS-CRITICAL phase — signed arithmetic in GAMEPLAY, behind an AIRTIGHT parent gate. Sets up a finished hero stepped through the glass with the negatives switch ON, and drives: the gate (signed combat problems DO appear in the mirror loop with the switch on); a REAL generated signed problem rendering "7 + (−3) = ?" with a clear unicode minus (no stray ASCII hyphen) and grading EXACTLY (typing "4" correct; typing the displayed "−3" for a negative answer accepted); a wrong SIGN ("8" for −4 × 2) graded gently WRONG — the worked solution "…= −8" shown, modal stays open, no punish; the Tweedle additive-inverse room (entering fires the Cheshire's NEGATIVE hint; walking −5 up into one socket then +5 into the other CANCELS the pair to zero and completes the room with the "Contrariwise" moment); and — the airtight half — with the switch OFF, 1600 mirror combat draws yield ZERO negatives and the Tweedle door shows the gentle "a grown-up can open negative numbers" note (never a locked wall, room not entered), and OUTSIDE the mirror even with the switch on there are ZERO negatives. Screenshots: the signed combat problem, the Tweedle room mid-solve (signed slabs + the ▢ + ▢ = 0 sockets), the cancel-to-zero moment, the switch-off note. Zero JS errors. (The EXACT signed-arithmetic table + fuzz, the negativesOn() gate both ways, the Tweedle socket predicate a+b===0 accepting every inverse pair and rejecting a non-zero sum, the switch persist/migrate-OFF, and the mirror round-trip carrying the switch state are also unit-tested in tests/test.js.)
- drive-numberline.js — Wave 23 "the number-line walk" (Looking Glass P3.5, ~24 checks): the deferred embodied-negatives FOUNDATION — negatives as a place your feet go, reusing the Wave 12 stepping-stone numeral-on-tile rendering. Sets up a finished hero stepped through the glass with negatives ON, and drives: the zero-meridian washing the mirror overworld into "the Below" (crossing west of MM.maps.MERIDIAN_X narrates the once-per-visit beat); entering the crossing fires the Cheshire's NEGATIVE hint; the crossing shows a walkable row of SIGNED stones (…−4 −3 −2 −1 [0] +1 +2 +3 +4…, zero marked apart, a clean unicode minus — the numeral-hazard watch); the signpost naming "Stand on −4"; WALKING the row (up onto stone 0, then west) so a wrong stone gives a gentle directional nudge that names it and never scolds/yanks, and stepping onto the −4 stone satisfies the target and advances; a RELATIVE target computing EXACTLY (anchored to the +2 stone you solved on → −1); completing the whole crossing showing the "the crossing is yours" moment + a small one-time reward; and — the gated half — with negatives OFF the crossing shows the gentle "a grown-up can open negative numbers" note (never a locked wall, crossing not entered), and OUTSIDE the mirror the door refuses ("a crossing that isn't here") so normal play has none of it. Screenshots: the meridian/Below wash, the crossing (the −4…0…+4 labels), the "Stand on −4" target prompt, the switch-off note. Zero JS errors. (The EXACT target logic — numberlineResolve's abs/zero/rel correctness table, a −4 target satisfied by the −4 stone and no other, the meridian beat gated both ways, the new-glyph tileSprite guards, and a pre-Wave-23 save unaffected — is also unit-tested in tests/test.js.)
- drive-finale.js — Looking Glass P4 finale: the completed-spiral reveal, the Jabberwocky mirror-writing plaque, and the Carroll cast (White Queen / Humpty / tea-party) all appear in the mirror and never in normal play.
