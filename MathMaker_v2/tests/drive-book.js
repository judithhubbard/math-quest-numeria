...
(async () => {
  ...
  check(/Discovered: 0 of 80/.test(body.replace(/\s+/g, ' ')), 'fresh book: 0 of 80 discovered');
  ...
  await page.waitForSelector('#modalBox h2');
  body = await page.textContent('#modalBox');
  check(/Discovered: 1 of 80/.test(body.replace(/\s+/g, ' ')), 'counter shows 1 of 80 after first kill');
